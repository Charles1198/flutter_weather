...
class DateUtil {
  ...
  static String timesAgo(String dateStr) {
    int ms = getDateMsByTimeStr(dateStr);
    int nowMs = getNowDateMs();
    if (nowMs <= ms) {
      return '刚刚';
    }
    int pastMs = nowMs - ms;
    int aSecondMs = 1000;
    int aMinuteMs = aSecondMs * 60;
    int anHourMs = aMinuteMs * 60;
    int aDayMs = anHourMs * 24;
    if (pastMs < aMinuteMs) {
      return '刚刚';
    } else if (pastMs < anHourMs) {
      int minutes =  pastMs ~/ aMinuteMs;
      return '$minutes 分钟前';
    } else if (pastMs < aDayMs) {
      int hours = pastMs ~/ anHourMs;
      return '$hours 小时前';
    } else {
      int days = pastMs ~/ aDayMs;
      return days <= 3 ? '$days 天前' : formatDateTime(dateStr, DateFormat.YEAR_MONTH_DAY, '-', '-');
    }
  }
}